...
核心思想：
  1. 把高维向量切成 M 段
  2. 每段独立做 K-Means，训练码本 codebook[m]  (假设每个码本256个子质心)
  3. 编码：每段子向量 → 找最近质心编号 (0~255) → 存 uint8
  4. 解码：uint8 编号 → 查码本 → 还原近似向量
  5. 距离：ADC 查表，不需要还原向量
  本质上 PQ 的核心先划分M个码本是，把"N 个不同向量的距离计算"收敛成"M × K* 个子质心的距离计算"
  之后在每个码本上用 K 子质心压缩表示所有待查询向量
  在计算距离时先划分成M段，在M个码本内计算与K个子质心的最短距离，并用这个子质心代表查询特征
  我们就获得了这个向量在M个不同码本的最近子质心的组合，这就是压缩后的PQ向量
  最终对所有向量数据库中的向量做PQ压缩表示，所有的向量都变成了PQ向量
"""
